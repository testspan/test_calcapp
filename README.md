...
Framework project to run WebdriverIO tests with [Cucumber](https://cucumber.io/) and brings **true** [BDD] to JavaScript. 

## Requirements

- Node version 6 or higher
- A preconfigured Selenium Grid, preinstalled browser driver or cloud provider account
- Use Yarn (>= 1.0.0), but feel free to replace this with NPM if that is what you are using.


## Features
- You can find them at the `/src/features` directory

## Tests

Tests are written in [Gherkin syntax](https://cucumber.io/docs/reference)
All test files are located in `./src/features/*` and have the file ending `.feature`. 
...
Execute a single feature file, to do so use the following command:
